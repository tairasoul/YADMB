        ...
        if (cg.audioPlayer.state.status == voice.AudioPlayerStatus.Idle) {
            ...
            switch (cg.loopType) {
                ...
                case "queue":
                    ...
                    if (cg.queuedTracks[currentTrack].trackNumber == cg.queuedTracks[currentTrack].tracks.length) cg.currentTrack += 1
...
const commands: Command[] = [
    ...
    },
    {
        ...
        .setDescription("Skip the current playlist. Will function the same as skip-song if the current track playing was not added through add-playlist.")
        .setDMPermission(false),
        async execute(interaction: oceanic.CommandInteraction) {
            await interaction.defer();
            const embed = new builders.EmbedBuilder();
